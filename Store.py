...
class Store:
    ...
    def product_search(self,search_term):
       ...
       for x in self._inventory:
        title = self._inventory[x].get_title()
        description = self._inventory[x].get_description()
    ...
    def add_product_to_member_cart(self,ID_code,account_ID):
        ...
        if ID_code not in self._inventory:
            return "product ID not found"
        if account_ID not in self._members:
            return "member ID not found"
        if self._inventory[ID_code].get_quantity_available()>0:
            self._members[account_ID].add_product_to_cart(ID_code)
    ...
    def check_out_member(self,account_ID):
        ...
        charge = 0
        account_ID = self._members[account_ID]
        for account_ID in account_ID.get_cart():
            product = self._inventory[ID_code]
